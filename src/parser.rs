...
#[derive(Debug)]
pub struct Parser<I: Iterator<Item = T>, T: Token> {
    tokens: Peekable<I>,
...
impl<I: Iterator<Item = T>, T: Token> Parser<I, T> {
    pub fn parse(tokens: I, mode: ParserMode) -> std::result::Result<Vec<ASTNode<T>>, ParseErrors> {
        ...
        while !matches!(parser.peek().token_type(), TokenType::Eof) {
            match parser.declaration() {
                Ok(stmt) => match *stmt {
                    Stmt::Expression(expr)
                        if matches!(parser.peek().token_type(), TokenType::Eof) =>
                    {
    ...
    //                | statement ;
    //
    fn declaration(&mut self) -> Result<Box<Stmt<T>>> {
    ...
    fn var_declaration(&mut self) -> Result<Box<Stmt<T>>> {
        // Consume 'var'
        self.next_token();
    ...
    fn fun_declaration(&mut self, kind: &'static str) -> Result<Box<Stmt<T>>> {
        // Consume 'fun'
        self.next_token();
        ...
        if !matches!(peeked.token_type(), TokenType::LeftBrace) {
            ExpectTokenSnafu {
                line: peeked.line(),
    ...
    // block          → "{" declaration* "}" ;
    //
    fn statement(&mut self) -> Result<Box<Stmt<T>>> {
        match self.peek().token_type() {
    ...
    fn print_statement(&mut self) -> Result<Box<Stmt<T>>> {
        // Consume 'print'
        self.next_token();
    ...
    fn block_statement(&mut self) -> Result<Box<Stmt<T>>> {
        // Consume '{'
        self.next_token();

        let mut statements: Vec<Stmt<T>> = Vec::new();
        while !matches!(
    ...
    fn if_statement(&mut self) -> Result<Box<Stmt<T>>> {
        // Consume 'if'
        self.next_token();
    ...
    fn while_statement(&mut self) -> Result<Box<Stmt<T>>> {
        // Consume 'while'
        self.next_token();
        self.loop_depth += 1;
    ...
    fn for_statement(&mut self) -> Result<Box<Stmt<T>>> {
        // Consume 'for'
        self.next_token();
        self.loop_depth += 1;
        ...
        let condition = if matches!(self.peek().token_type(), TokenType::Semicolon) {
            None
        } else {
        ...
        let increment = if matches!(self.peek().token_type(), TokenType::RightParen) {
            None
        } else {
    ...
    fn return_statement(&mut self) -> Result<Box<Stmt<T>>> {
        // Consume 'return'
    ...
    fn break_statement(&mut self) -> Result<Box<Stmt<T>>> {
        // Consume 'break'
        let token = self.next_token();
    ...
    fn expression_statement(&mut self) -> Result<Box<Stmt<T>>> {
        let expr = self.expression()?;
        // REPL Mode:
        // Accept lack of semicolon in expression statement if it is last
        if !matches!(self.mode, ParserMode::Repl)
    ...
    //                | "super" "." IDENTIFIER ;
    //
    fn expression(&mut self) -> Result<Box<Expr<T>>> {
        self.assignment()
    }

    fn assignment(&mut self) -> Result<Box<Expr<T>>> {
    ...
    fn logic_or(&mut self) -> Result<Box<Expr<T>>> {
    ...
    fn logic_and(&mut self) -> Result<Box<Expr<T>>> {
    ...
    fn equality(&mut self) -> Result<Box<Expr<T>>> {
    ...
    fn comparison(&mut self) -> Result<Box<Expr<T>>> {
        self.parse_binary_op(
            |p| p.term(),
            &[
                TokenType::Greater,
    ...
    fn call(&mut self) -> Result<Box<Expr<T>>> {
    ...
    fn primary(&mut self) -> Result<Box<Expr<T>>> {
        match self.peek().token_type() {
            TokenType::True
            | TokenType::False
            | TokenType::Nil
            ...
            | TokenType::String => Ok(Box::new(Expr::Literal {
                value: self.next_token().literal().expect("literal exist").clone(),
            })),
            TokenType::LeftParen => {
                self.next_token();
                let expr = self.expression()?;
    ...
    fn parse_binary_op(
        &mut self,
        parse_operand: fn(&mut Self) -> Result<Box<Expr<T>>>,
        operators: &[TokenType],
    ) -> Result<Box<Expr<T>>> {
    ...
    fn finish_call(&mut self, callee: Box<Expr<T>>) -> Result<Box<Expr<T>>> {
        // Consume '('
        self.next_token();
    ...
    fn next_if(&mut self, match_type: TokenType) -> Option<T> {
    ...
    fn expect_next(&mut self, match_type: TokenType, expect_desc: &str) -> Result<T> {
        match self.tokens.peek() {
